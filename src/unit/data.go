...
func setup() []*unit {
	...
	unitless := def(expMap{})
	acceleration := def(expMap{meter: 1, second: -2})
	angle := def(expMap{radian: 1})
	angularVelocity := def(expMap{radian: 1, second: -1})
	area := def(expMap{meter: 2})
	density := def(expMap{kilogram: 1, meter: -3})
	duration := def(expMap{second: 1})
	electricCurrent := def(expMap{ampere: 1})
	electricCharge := def(expMap{ampere: 1, second: 1})
	energy := def(expMap{kilogram: 1, meter: 2, second: -2})
	force := def(expMap{kilogram: 1, meter: 1, second: -2})
	frequency := def(expMap{second: -1})
	fuelEfficiency := def(expMap{meter: 2})
	illuminance := def(expMap{candela: 1, steradian: 1, meter: -2})
	information := def(expMap{byte: 1})
	length := def(expMap{meter: 1})
	...
	power := def(expMap{kilogram: 1, meter: 2, second: -3})
	pressure := def(expMap{kilogram: 1, meter: -1, second: -2})
	solidAngle := def(expMap{steradian: 1})
	speed := def(expMap{meter: 1, second: -1})
	temperature := def(expMap{kelvin: 1})
	voltage := def(expMap{meter: 2, kilogram: 1, second: -3, ampere: -1})
	volume := def(expMap{meter: 3})
