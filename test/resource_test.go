package t

import (
	"testing"
	"unit"
	"unit/resource"
)

func TestNewHeap(t *testing.T) {
	rsc := resource.New(unit.Q(1, "kg"), unit.Q(100, "kg"), "")
	if rsc == nil {
		t.Error("failed heap creation")
	}
}

func TestDeposit(t *testing.T) {
	rsc := resource.New(unit.Q(1, "kg"), unit.Q(100, "kg"), "")
	ok := rsc.Deposit(unit.Q(50, "m2"))
	if ok {
		t.Error("incompatibility ignored")
	}
	if ok = rsc.Deposit(unit.Q(-20, "kg")); ok {
		t.Error("min ignored")
	}
	if ok = rsc.Deposit(unit.Q(200, "kg")); ok {
		t.Error("max ignored")
	}
	if !unit.Equal(rsc.Balance(), unit.Q(1, "kg"), unit.Q(1, "g")) {
		t.Error("not equal", rsc.Balance(), "1 kg")
	}
	rsc.Deposit(unit.Q(150, "g"))
	if !unit.Equal(rsc.Balance(), unit.Q(1150, "g"), unit.Q(1, "g")) {
		t.Error("balance wrong", rsc.Balance())
	}
}

func TestWithdraw(t *testing.T) {
	rsc := resource.New(unit.Q(-1, "kWh"), unit.Q(100, "kWh"), "")
	if rsc.Set(unit.Q(150, "kWh")) {
		t.Error("ignored out of bounds")
	}
	rsc.Set(unit.Q(50, "kWh"))
	rsc.Withdraw(unit.Q(1e6, "J"))
	if !unit.Equal(rsc.Balance(), unit.Q(1.79e8, "m2.kg/s2"), unit.Q(1, "J")) {
		t.Error("balance wrong", rsc.Balance())
	}
	if rsc.Withdraw(unit.Q(90, "kWh")) {
		t.Error("min ignored")
	}
}

func TestMinMax(t *testing.T) {
	rsc := resource.New(unit.Q(0, "m"), unit.Q(100, "m"), "")
	rsc.Set(unit.Q(30, "m"))
	if rsc.Min(unit.Q(31, "m")) {
		t.Error("min more than balance accepted")
	}
	if rsc.Max(unit.Q(29, "m")) {
		t.Error("max less than balance accepted")
	}
	min, max := rsc.Limits()
	if !unit.Equal(min, unit.Q(0, "m"), unit.Q(1, "cm")) {
		t.Error("lower bound wrong")
	}
	if !unit.Equal(max, unit.Q(100, "m"), unit.Q(1, "cm")) {
		t.Error("upper bound wrong")
	}
}

func TestWithdrawPctContext(t *testing.T) {
	unit.DefineContext("tank", "L", "%.1[1]fℓ")
	rsc := resource.New(unit.Q(1, "L"), unit.Q(50, "L"), "tank")
	rsc.Set(unit.Q(20, "L"))
	m, err := rsc.WithdrawPct(25)
	if err != nil || m.String() != "5.0000 L" {
		t.Error("expected: 5.0000 L, actual:", m.String())
	}
	s := rsc.Balance().String()
	if s != "15.0000 L" {
		t.Error("balance not reduced to 15.0000 L:", s)
	}
	m, err = rsc.WithdrawPct(200)
	if err == nil {
		t.Error("should not be allowed to withdraw 200%")
	}
	s = rsc.String()
	if s != "15.0ℓ" {
		t.Error("value withdrawn despite being invalid")
	}
}
